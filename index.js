...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    passwordString += characters[charactersIndex];
  }
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof (a, b) === "number") {
    ...
    let result = Math.sqrt(powA + powB);
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof (radius, height) === "number") {
    let radiusPow = Math.pow(radius, 2);

    let volumeCylinder = Math.PI * radiusPow * height;

    let result = Math.ceil(volumeCylinder);
...
function sumPositiveNumbers(numbers) {
  ...
  if (!Array.isArray(numbers)) {
    return "Помилка: аргумент має бути масивом чисел", null;
  } else {
    let positivValue = 0;

    for (let elements of numbers) {
      if (Math.sign(elements) > 0) {
        positivValue += elements;
      }
    }
...
function getFractionalPart(num) {
  ...
  if (typeof num !== "number") {
    return "Помилка: вхідний аргумент має бути числом.", null;
  } else {
    let truncNum = Math.trunc(num);

    let nextStep = num - truncNum;

    let result = Math.fround(nextStep);
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (typeof (principal, interestRate, years) !== "number") {
    console.log("Усі значення повинні бути числами");
    return null;
  } else {
    let double = Math.pow(1 + interestRate, years);
    let calculate = principal * double;
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    console.log("Повиннне бути массивом");
  }

  if (typeof maxPrice !== "number") {
    return null;
  } else {
    let productSum = products.reduce(
      (accumulator, currentValue) => accumulator + currentValue,
      0
    );

    let roundPrice = Math.round(productSum);
    let roundMax = Math.round(maxPrice);

    let compare = roundPrice > roundMax;
